...
public class ProblemBSolver {
	...
	public static String calc(int N, long K, int[] c, int[] t, int[] s) {
		print("N={" + N + "}, K={" + K + "}");
		...
		for (int i = 0; i < K; i++) {
			...
			chosenCoffee = -1;
			for (int j = 0; j < N; j++) {
				if (c[j] == 0 || t[j] < i + 1) continue;
				if (chosenCoffee == -1 || s[chosenCoffee] < s[j]) {
					chosenCoffee = j;
				}
			}
			...
			// drink
			c[chosenCoffee]--;
			totalS += s[chosenCoffee];
			print("choose: " + chosenCoffee + ", total: " + totalS);
		}
		return totalS + "";
	}
	...
	public static void exec(String fileName) {
		...
		for (i = 0; i < mNumCase; i++) {
			...
			for (int j = 0; j < N; j++) {
				tmpStr = readLine();
				strArray = tmpStr.split(" ");
				c[j] = Integer.parseInt(strArray[0]);
				t[j] = Integer.parseInt(strArray[1]);
				s[j] = Integer.parseInt(strArray[2]);
			}
			ans = calc(N, K, c, t, s);
